            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
                            ...
                            <form wire:submit.prevent="store">
                                <div class="grid grid-cols-1 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input id="curso" type="text" wire:model="curso"
                                        ...
                                        <x-input-error for="curso" class="mt-2" />
                                    </div>
                                    <!-- Unidade -->
                                    <div>
                                        <label for="unidade"
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Unidade</label>
                                        <input id="unidade" type="text" wire:model="unidade"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
                                    ...
                                    <div>
                                        <label for="hora"
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Hora</label>
                                        <input id="hora" type="text" wire:model="hora"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
                                            placeholder="ex: 19:00" required>
                                        <x-input-error for="hora" class="mt-2" />
                                    </div>
                                    <!-- Checklist -->
                                    <div>
                                        <label for="checklist"
                                            class="block text-sm font-medium text-gray-700 dark:text-gray-300">Checklist</label>
                                        <textarea id="checklist" wire:model="checklist"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200"
                                            placeholder="Checklist"></textarea>
                                        <x-input-error for="checklist" class="mt-2" />
                                    </div>
                                </div>
                        ...
                        @if ($selectedFormatura)
                            <div
                                ...
                                <div class="mb-6">
                                    ...
                                    <p class="text-gray-700 dark:text-gray-300"><strong>Unidade:</strong>
                                        {{ $selectedFormatura->unidade }}</p>
                                    <p class="text-gray-700 dark:text-gray-300"><strong>Data:</strong>
                                        {{ \Carbon\Carbon::parse($selectedFormatura->data)->format('d/m/Y') }}</p>
                                    <p class="text-gray-700 dark:text-gray-300"><strong>Hora:</strong>
                                        {{ $selectedFormatura->hora }}</p>
                                    <p class="text-gray-700 dark:text-gray-300"><strong>Checklist:</strong>
                                        {{ $selectedFormatura->checklist ?? 'Não informado' }}</p>
                                </div>
                            </div>
                        @else
                ...
                @if (!empty($results))
                        ...
                        <table class="w-full table-auto border-collapse">
                            <thead>
                                <tr class="bg-gray-100 dark:bg-gray-700">
                                    <th scope="col" wire:click="sortBy('curso')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Curso @if ($sortField === 'curso')
                                            {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </th>
                                    <th scope="col" wire:click="sortBy('unidade')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Unidade @if ($sortField === 'unidade')
                                            {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </th>
                                    <th scope="col" wire:click="sortBy('data')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Data @if ($sortField === 'data')
                                            {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </th>
                                    <th scope="col" wire:click="sortBy('hora')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Hora @if ($sortField === 'hora')
                                            {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </th>
                                    <th scope="col" wire:click="sortBy('checklist')"
                                        class="px-4 py-3 text-left text-sm font-medium text-gray-700 dark:text-gray-200 cursor-pointer">
                                        Checklist @if ($sortField === 'checklist')
                                            {{ $sortDirection === 'asc' ? '↑' : '↓' }}
                                        @endif
                                    </th>
                                    <th scope="col"
                            ...
                            <tbody>
                                @foreach ($results as $index => $formatura)
                                    <tr wire:key="table-row-{{ $formatura->id }}-{{ $index }}"
                                        ...
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200 truncate max-w-[150px]"
                                            title="{{ $formatura->curso }}">{{ $formatura->curso }}</td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200 truncate max-w-[150px]"
                                            title="{{ $formatura->unidade }}">{{ $formatura->unidade }}</td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                            {{ \Carbon\Carbon::parse($formatura->data)->format('d/m/Y') }}</td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200">
                                            {{ $formatura->hora }}</td>
                                        <td class="border px-4 py-3 text-sm text-gray-700 dark:text-gray-200 truncate max-w-[200px]"
                                            title="{{ $formatura->checklist }}">{{ $formatura->checklist ?? 'N/A' }}
                                        </td>
                    ...
                    <div class="md:hidden space-y-4">
                        @foreach ($results as $index => $formatura)
                            <div wire:key="card-{{ $formatura->id }}-{{ $index }}"
                                class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 shadow-sm">
                                <div class="flex justify-between items-center">
                                    <div>
                                        <p class="text-lg font-semibold text-gray-900 dark:text-gray-100">
                                            {{ $formatura->curso }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">Unidade:
                                            {{ $formatura->unidade }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">Data:
                                            {{ \Carbon\Carbon::parse($formatura->data)->format('d/m/Y') }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">Hora:
                                            {{ $formatura->hora }}</p>
                                        <p class="text-sm text-gray-600 dark:text-gray-400">Checklist:
                                            {{ $formatura->checklist ?? 'N/A' }}</p>
                                    </div>
                ...
                @else
                    <p class="text-gray-500 dark:text-gray-400 text-sm mt-4">Nenhuma formatura encontrada. Use o campo
